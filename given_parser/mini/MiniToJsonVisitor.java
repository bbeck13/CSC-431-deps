...
public class MiniToJsonVisitor extends MiniBaseVisitor<JsonValue> {
   ...
   public JsonValue visitProgram(MiniParser.ProgramContext ctx) {
      return factory.createObjectBuilder().add("types", visit(ctx.types()))
            .add("declarations", visit(ctx.declarations()))
            .add("functions", visit(ctx.functions())).build();
   }
   ...
   public JsonValue visitTypeDeclaration(MiniParser.TypeDeclarationContext ctx) {
      return factory.createObjectBuilder().add("line", ctx.getStart().getLine())
            .add("id", ctx.ID().getText()).add("fields", visit(ctx.nestedDecl())).build();
   }
   ...
   public JsonValue visitDecl(MiniParser.DeclContext ctx) {
      return factory.createObjectBuilder().add("line", ctx.getStart().getLine())
            .add("type", visit(ctx.type())).add("id", ctx.ID().getText()).build();
   }
   ...
   private void addDeclarationsTo(MiniParser.DeclarationContext ctx,
      ...
      for (TerminalNode node : ctx.ID()) {
         abuilder
               .add(factory.createObjectBuilder().add("line", node.getSymbol().getLine())
                     .add("type", type).add("id", node.getText()));
      }
   }
   ...
   public JsonValue visitFunction(MiniParser.FunctionContext ctx) {
      return factory.createObjectBuilder().add("line", ctx.getStart().getLine())
            .add("id", ctx.ID().getText()).add("parameters", visit(ctx.parameters()))
            .add("return_type", visit(ctx.returnType()))
            .add("declarations", visit(ctx.declarations()))
            .add("body", visit(ctx.statementList())).build();
   }
   ...
   public JsonValue visitAssignment(MiniParser.AssignmentContext ctx) {
      ...
      if (ctx.expression() != null) {
         ...
         expression = factory.createObjectBuilder().add("line", ctx.getStart().getLine())
               .add("exp", "read").build();
      }

      return factory.createObjectBuilder().add("line", ctx.getStart().getLine())
            .add("stmt", "assign").add("source", expression)
            .add("target", visit(ctx.lvalue())).build();
   }

   @Override
   public JsonValue visitPrint(MiniParser.PrintContext ctx) {
      return factory.createObjectBuilder().add("line", ctx.getStart().getLine())
            .add("stmt", "print").add("exp", visit(ctx.expression())).add("endl", false)
            .build();
   }
   ...
   public JsonValue visitPrintLn(MiniParser.PrintLnContext ctx) {
      return factory.createObjectBuilder().add("line", ctx.getStart().getLine())
            .add("stmt", "print").add("exp", visit(ctx.expression())).add("endl", true)
            .build();
   }
   ...
   public JsonValue visitConditional(MiniParser.ConditionalContext ctx) {
      JsonObjectBuilder obuilder = factory.createObjectBuilder()
            .add("line", ctx.getStart().getLine()).add("stmt", "if")
            .add("guard", visit(ctx.expression())).add("then", visit(ctx.thenBlock));
   ...
   public JsonValue visitWhile(MiniParser.WhileContext ctx) {
      return factory.createObjectBuilder().add("line", ctx.getStart().getLine())
            .add("stmt", "while").add("guard", visit(ctx.expression()))
            .add("body", visit(ctx.block())).build();
   }

   @Override
   public JsonValue visitDelete(MiniParser.DeleteContext ctx) {
      return factory.createObjectBuilder().add("line", ctx.getStart().getLine())
            .add("stmt", "delete").add("exp", visit(ctx.expression())).build();
   }

   @Override
   public JsonValue visitReturn(MiniParser.ReturnContext ctx) {
      JsonObjectBuilder obuilder = factory.createObjectBuilder()
            .add("line", ctx.getStart().getLine()).add("stmt", "return");
   ...
   public JsonValue visitInvocation(MiniParser.InvocationContext ctx) {
      return factory.createObjectBuilder().add("line", ctx.getStart().getLine())
            .add("stmt", "invocation").add("id", ctx.ID().getText())
            .add("args", visit(ctx.arguments())).build();
   }
   ...
   public JsonValue visitIdentifierExpr(MiniParser.IdentifierExprContext ctx) {
      return factory.createObjectBuilder().add("line", ctx.getStart().getLine())
            .add("exp", "id").add("id", ctx.ID().getText()).build();
   }

   @Override
   public JsonValue visitBinaryExpr(MiniParser.BinaryExprContext ctx) {
      return factory.createObjectBuilder().add("line", ctx.op.getLine())
            .add("exp", "binary").add("operator", ctx.op.getText())
            .add("lft", visit(ctx.lft)).add("rht", visit(ctx.rht)).build();
   }

   @Override
   public JsonValue visitNewExpr(MiniParser.NewExprContext ctx) {
      return factory.createObjectBuilder().add("line", ctx.getStart().getLine())
            .add("exp", "new").add("id", ctx.ID().getText()).build();
   }

   @Override
   public JsonValue visitDotExpr(MiniParser.DotExprContext ctx) {
      return factory.createObjectBuilder().add("line", ctx.getStart().getLine())
            .add("exp", "dot").add("left", visit(ctx.expression()))
            .add("id", ctx.ID().getText()).build();
   }
   ...
   public JsonValue visitUnaryExpr(MiniParser.UnaryExprContext ctx) {
      ...
      if (doubleNegatingUnary(op, opnd)) {
         ...
         return factory.createObjectBuilder().add("line", ctx.getStart().getLine())
               .add("exp", "unary").add("operator", op).add("operand", visit(opnd))
               .build();
      }
   }
   ...
   public JsonValue visitInvocationExpr(MiniParser.InvocationExprContext ctx) {
      return factory.createObjectBuilder().add("line", ctx.getStart().getLine())
            .add("exp", "invocation").add("id", ctx.ID().getText())
            .add("args", visit(ctx.arguments())).build();
   }
   ...
   public JsonValue visitBlock(MiniParser.BlockContext ctx) {
      return factory.createObjectBuilder().add("stmt", "block")
            .add("list", visit(ctx.statementList())).build();
   }
