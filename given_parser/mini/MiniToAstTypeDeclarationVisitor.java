...
import mini.ast.TypeDeclaration;

public class MiniToAstTypeDeclarationVisitor extends MiniBaseVisitor<TypeDeclaration> {
   private final MiniToAstTypeVisitor typeVisitor = new MiniToAstTypeVisitor();

   @Override
   public TypeDeclaration visitTypeDeclaration(MiniParser.TypeDeclarationContext ctx) {
      return new TypeDeclaration(ctx.getStart().getLine(), ctx.ID().getText(),
            gatherFieldDeclarations(ctx.nestedDecl()));
   }

   private List<Declaration> gatherFieldDeclarations(MiniParser.NestedDeclContext ctx) {
