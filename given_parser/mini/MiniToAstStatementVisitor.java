...
import mini.ast.AssignmentStatement;
import mini.ast.BlockStatement;
import mini.ast.ConditionalStatement;
import mini.ast.DeleteStatement;
import mini.ast.DotExpression;
import mini.ast.Expression;
import mini.ast.IdentifierExpression;
import mini.ast.InvocationExpression;
import mini.ast.InvocationStatement;
import mini.ast.Lvalue;
import mini.ast.LvalueDot;
import mini.ast.LvalueId;
import mini.ast.PrintLnStatement;
import mini.ast.PrintStatement;
import mini.ast.ReadExpression;
import mini.ast.ReturnEmptyStatement;
import mini.ast.ReturnStatement;
import mini.ast.Statement;
import mini.ast.WhileStatement;

public class MiniToAstStatementVisitor extends MiniBaseVisitor<Statement> {
   private final MiniToAstExpressionVisitor expressionVisitor = new MiniToAstExpressionVisitor();
   ...
   public Statement visitAssignment(MiniParser.AssignmentContext ctx) {
      ...
      return new AssignmentStatement(ctx.getStart().getLine(), visitLvalue(ctx.lvalue()),
            expression);
   }

   @Override
   public Statement visitPrint(MiniParser.PrintContext ctx) {
      return new PrintStatement(ctx.getStart().getLine(),
            expressionVisitor.visit(ctx.expression()));
   }

   @Override
   public Statement visitPrintLn(MiniParser.PrintLnContext ctx) {
      return new PrintLnStatement(ctx.getStart().getLine(),
            expressionVisitor.visit(ctx.expression()));
   }

   @Override
   public Statement visitConditional(MiniParser.ConditionalContext ctx) {
      return new ConditionalStatement(ctx.getStart().getLine(),
            expressionVisitor.visit(ctx.expression()), visit(ctx.thenBlock),
            ctx.elseBlock != null ? visit(ctx.elseBlock) : BlockStatement.emptyBlock());
   }

   @Override
   public Statement visitWhile(MiniParser.WhileContext ctx) {
      return new WhileStatement(ctx.getStart().getLine(),
            expressionVisitor.visit(ctx.expression()), visit(ctx.block()));
   }

   @Override
   public Statement visitDelete(MiniParser.DeleteContext ctx) {
      return new DeleteStatement(ctx.getStart().getLine(),
            expressionVisitor.visit(ctx.expression()));
   }
   ...
   public Statement visitInvocation(MiniParser.InvocationContext ctx) {
      return new InvocationStatement(ctx.getStart().getLine(),
            new InvocationExpression(ctx.getStart().getLine(), ctx.ID().getText(),
                  gatherArguments(ctx.arguments())));
   }

   private List<Expression> gatherArguments(MiniParser.ArgumentsContext ctx) {
   ...
   private Lvalue visitLvalue(MiniParser.LvalueContext ctx) {
      if (ctx instanceof MiniParser.LvalueIdContext) {
         ...
         return new LvalueId(lctx.getStart().getLine(), lctx.ID().getText());
      } else {
         MiniParser.LvalueDotContext lctx = (MiniParser.LvalueDotContext) ctx;
         return new LvalueDot(lctx.getStart().getLine(), visitLvalueNested(lctx.lvalue()),
               lctx.ID().getText());
      }
   }
   ...
   private Expression visitLvalueNested(MiniParser.LvalueContext ctx) {
      if (ctx instanceof MiniParser.LvalueIdContext) {
         MiniParser.LvalueIdContext lctx = (MiniParser.LvalueIdContext) ctx;
         return new IdentifierExpression(lctx.getStart().getLine(), lctx.ID().getText());
      } else {
         MiniParser.LvalueDotContext lctx = (MiniParser.LvalueDotContext) ctx;
